...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Deleting field 'Feed.has_exception'
        db.delete_column('feeds', 'has_exception')

        # Adding index on 'Feed', fields ['has_page_exception']
        db.create_index('feeds', ['has_page_exception'])

        # Adding index on 'Feed', fields ['has_feed_exception']
        db.create_index('feeds', ['has_feed_exception'])


    def backwards(self, orm):
        
        # Removing index on 'Feed', fields ['has_feed_exception']
        db.delete_index('feeds', ['has_feed_exception'])

        # Removing index on 'Feed', fields ['has_page_exception']
        db.delete_index('feeds', ['has_page_exception'])

        # Adding field 'Feed.has_exception'
        db.add_column('feeds', 'has_exception', self.gf('django.db.models.fields.BooleanField')(default=False), keep_default=False)
