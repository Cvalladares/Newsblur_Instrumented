...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding field 'DuplicateFeed.duplicate_link'
        db.add_column('rss_feeds_duplicatefeed', 'duplicate_link', self.gf('django.db.models.fields.CharField')(max_length=255, null=True, db_index=True), keep_default=False)

        # Adding index on 'DuplicateFeed', fields ['duplicate_feed_id']
        db.create_index('rss_feeds_duplicatefeed', ['duplicate_feed_id'])

        # Adding index on 'DuplicateFeed', fields ['duplicate_address']
        db.create_index('rss_feeds_duplicatefeed', ['duplicate_address'])


    def backwards(self, orm):
        
        # Removing index on 'DuplicateFeed', fields ['duplicate_address']
        db.delete_index('rss_feeds_duplicatefeed', ['duplicate_address'])

        # Removing index on 'DuplicateFeed', fields ['duplicate_feed_id']
        db.delete_index('rss_feeds_duplicatefeed', ['duplicate_feed_id'])

        # Deleting field 'DuplicateFeed.duplicate_link'
        db.delete_column('rss_feeds_duplicatefeed', 'duplicate_link')
